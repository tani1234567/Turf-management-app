...
export default function ExpenseTrackingScreen({ navigation }) {
  ...
  const categories = getCategoriesForRole("caretaker");
  ...
  const fetchExpenses = useCallback(async () => {
    const turfId = user?.assignedTurfId;
    if (!turfId) {
      setLoading(false);
      return;
    }
    ...
    try {
      const data = await getExpensesForTurf(turfId, {
        addedBy: user.userId || user.uid,
      });
      setExpenses(data);
  ...
  }, [user?.assignedTurfId, user?.userId, user?.uid]);
    ...
    try {
      if (editingExpense) {
        ...
        const updates = {
          ...
          categoryLabel: getCategoryLabel(selectedCategory, "caretaker"),
          amount: parseFloat(amount),
          description: description.trim(),
          date,
        };

        // Handle new images for edit (only upload new local URIs)
  ...
  const renderExpenseItem = ({ item }) => {
    const iconName = getCategoryIcon(item.category, "caretaker");
    const categoryLabel = getCategoryLabel(item.category, "caretaker");
    const hasReceipt =
                  ...
                  {categories.map((cat) => (
                    ...
                    >
                      <MaterialCommunityIcons
                        ...
                        size={18}
                        color={
                          selectedCategory === cat.id ? "#fff" : CARETAKER_COLOR
                        }
                      />
                      <Text
                        style={[
